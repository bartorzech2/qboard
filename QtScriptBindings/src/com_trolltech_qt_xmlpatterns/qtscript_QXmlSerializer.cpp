...
QScriptValue qtscript_create_QXmlSerializer_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 4; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QXmlSerializer_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QXmlSerializer_static_call, proto, function_lengths[0]);
