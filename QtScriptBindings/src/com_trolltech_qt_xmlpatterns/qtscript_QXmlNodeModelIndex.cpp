...
Q_DECLARE_METATYPE(QXmlNodeModelIndex::Axis)
...
static const QXmlNodeModelIndex::Axis qtscript_QXmlNodeModelIndex_Axis_values[] = {
...
static const char * const qtscript_QXmlNodeModelIndex_Axis_keys[] = {
    "AxisChild"
    , "AxisDescendant"
    , "AxisAttribute"
    , "AxisSelf"
...
static QString qtscript_QXmlNodeModelIndex_Axis_toStringHelper(QXmlNodeModelIndex::Axis value)
{
    for (int i = 0; i < 13; ++i) {
        if (qtscript_QXmlNodeModelIndex_Axis_values[i] == value)
            return QString::fromLatin1(qtscript_QXmlNodeModelIndex_Axis_keys[i]);
    }
    return QString();
}

static QScriptValue qtscript_QXmlNodeModelIndex_Axis_toScriptValue(QScriptEngine *engine, const QXmlNodeModelIndex::Axis &value)
{
    QScriptValue clazz = engine->globalObject().property(QString::fromLatin1("QXmlNodeModelIndex"));
    return clazz.property(qtscript_QXmlNodeModelIndex_Axis_toStringHelper(value));
}

static void qtscript_QXmlNodeModelIndex_Axis_fromScriptValue(const QScriptValue &value, QXmlNodeModelIndex::Axis &out)
{
    out = qvariant_cast<QXmlNodeModelIndex::Axis>(value.toVariant());
}

static QScriptValue qtscript_construct_QXmlNodeModelIndex_Axis(QScriptContext *context, QScriptEngine *engine)
{
    int arg = context->argument(0).toInt32();
    for (int i = 0; i < 13; ++i) {
        if (qtscript_QXmlNodeModelIndex_Axis_values[i] == arg)
            return qScriptValueFromValue(engine,  static_cast<QXmlNodeModelIndex::Axis>(arg));
    }
    return context->throwError(QString::fromLatin1("Axis(): invalid enum value (%0)").arg(arg));
}

static QScriptValue qtscript_QXmlNodeModelIndex_Axis_valueOf(QScriptContext *context, QScriptEngine *engine)
{
    QXmlNodeModelIndex::Axis value = qscriptvalue_cast<QXmlNodeModelIndex::Axis>(context->thisObject());
    return QScriptValue(engine, static_cast<int>(value));
}

static QScriptValue qtscript_QXmlNodeModelIndex_Axis_toString(QScriptContext *context, QScriptEngine *engine)
{
    QXmlNodeModelIndex::Axis value = qscriptvalue_cast<QXmlNodeModelIndex::Axis>(context->thisObject());
    return QScriptValue(engine, qtscript_QXmlNodeModelIndex_Axis_toStringHelper(value));
}

static QScriptValue qtscript_create_QXmlNodeModelIndex_Axis_class(QScriptEngine *engine, QScriptValue &clazz)
{
    QScriptValue ctor = qtscript_create_enum_class_helper(
        engine, qtscript_construct_QXmlNodeModelIndex_Axis,
        qtscript_QXmlNodeModelIndex_Axis_valueOf, qtscript_QXmlNodeModelIndex_Axis_toString);
    qScriptRegisterMetaType<QXmlNodeModelIndex::Axis>(engine, qtscript_QXmlNodeModelIndex_Axis_toScriptValue,
        qtscript_QXmlNodeModelIndex_Axis_fromScriptValue, ctor.property(QString::fromLatin1("prototype")));
    for (int i = 0; i < 13; ++i) {
        clazz.setProperty(QString::fromLatin1(qtscript_QXmlNodeModelIndex_Axis_keys[i]),
            engine->newVariant(qVariantFromValue(qtscript_QXmlNodeModelIndex_Axis_values[i])),
            QScriptValue::ReadOnly | QScriptValue::Undeletable);
    }
    return ctor;
}
...
QScriptValue qtscript_create_QXmlNodeModelIndex_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 6; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QXmlNodeModelIndex_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QXmlNodeModelIndex_static_call, proto, function_lengths[0]);
    ...
    ctor.setProperty(QString::fromLatin1("Axis"),
        qtscript_create_QXmlNodeModelIndex_Axis_class(engine, ctor));
    return ctor;
}
