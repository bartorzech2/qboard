...
QScriptValue qtscript_create_QXmlFormatter_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 3; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QXmlFormatter_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QXmlFormatter_static_call, proto, function_lengths[0]);
