...
QScriptValue qtscript_create_QNetworkCookie_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 16; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QNetworkCookie_prototype_call, function_lengths[i+2]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QNetworkCookie_static_call, proto, function_lengths[0]);
