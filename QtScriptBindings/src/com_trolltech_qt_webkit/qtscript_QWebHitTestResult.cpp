...
QScriptValue qtscript_create_QWebHitTestResult_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 14; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QWebHitTestResult_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QWebHitTestResult_static_call, proto, function_lengths[0]);
