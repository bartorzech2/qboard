...
QScriptValue qtscript_create_QDomNodeList_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 8; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QDomNodeList_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QDomNodeList_static_call, proto, function_lengths[0]);
