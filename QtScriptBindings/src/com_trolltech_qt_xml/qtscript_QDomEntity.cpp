...
QScriptValue qtscript_create_QDomEntity_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 4; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QDomEntity_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QDomEntity_static_call, proto, function_lengths[0]);
