...
QScriptValue qtscript_create_QDomDocumentType_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 7; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QDomDocumentType_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QDomDocumentType_static_call, proto, function_lengths[0]);
