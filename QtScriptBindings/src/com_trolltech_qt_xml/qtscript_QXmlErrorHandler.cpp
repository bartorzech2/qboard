...
QScriptValue qtscript_create_QXmlErrorHandler_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 5; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QXmlErrorHandler_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QXmlErrorHandler_static_call, proto, function_lengths[0]);
