...
static QScriptValue qtscript_QDomElement_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (context->argumentCount() == 2) {
        if (context->argument(0).isString()
            ...
            && (qMetaTypeId<float>() == context->argument(1).toVariant().userType())) {
            QString _q_arg0 = context->argument(0).toString();
            ...
            && (qMetaTypeId<qulonglong>() == context->argument(1).toVariant().userType())) {
            QString _q_arg0 = context->argument(0).toString();
            qulonglong _q_arg1 = qscriptvalue_cast<qulonglong>(context->argument(1));
            _q_self->setAttribute(_q_arg0, _q_arg1);
            return context->engine()->undefinedValue();
        }
    }
    ...
    if (context->argumentCount() == 3) {
        if (context->argument(0).isString()
            ...
            && context->argument(1).isString()
            && (qMetaTypeId<qulonglong>() == context->argument(2).toVariant().userType())) {
            QString _q_arg0 = context->argument(0).toString();
            QString _q_arg1 = context->argument(1).toString();
            qulonglong _q_arg2 = qscriptvalue_cast<qulonglong>(context->argument(2));
            _q_self->setAttributeNS(_q_arg0, _q_arg1, _q_arg2);
            return context->engine()->undefinedValue();
        }
    }
...
QScriptValue qtscript_create_QDomElement_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 20; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QDomElement_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QDomElement_static_call, proto, function_lengths[0]);
