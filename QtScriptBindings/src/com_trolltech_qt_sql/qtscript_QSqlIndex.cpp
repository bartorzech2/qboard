...
QScriptValue qtscript_create_QSqlIndex_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 8; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QSqlIndex_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QSqlIndex_static_call, proto, function_lengths[0]);
