...
static QScriptValue qtscript_QSqlTableModel_static_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (context->argumentCount() == 0) {
        ...
        QScriptValue _q_result = context->engine()->newVariant(context->thisObject(), qVariantFromValue((QSqlTableModel*)_q_cpp_result));
...
QScriptValue qtscript_create_QSqlTableModel_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 21; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QSqlTableModel_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QSqlTableModel_static_call, proto, function_lengths[0]);
