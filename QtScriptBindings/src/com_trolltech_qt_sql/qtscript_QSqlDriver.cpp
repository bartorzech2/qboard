...
QScriptValue qtscript_create_QSqlDriver_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 21; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QSqlDriver_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QSqlDriver_static_call, proto, function_lengths[0]);
