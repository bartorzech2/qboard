...
QScriptValue qtscript_create_QSqlResult_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 2; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QSqlResult_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QSqlResult_static_call, proto, function_lengths[0]);
