...
static QScriptValue qtscript_QSqlRelationalTableModel_static_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (context->argumentCount() == 0) {
        ...
        QScriptValue _q_result = context->engine()->newVariant(context->thisObject(), qVariantFromValue((QSqlRelationalTableModel*)_q_cpp_result));
...
QScriptValue qtscript_create_QSqlRelationalTableModel_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 4; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QSqlRelationalTableModel_prototype_call, function_lengths[i+1]);
        fun.setData(QScriptValue(engine, uint(0xBABE0000 + i)));
        proto.setProperty(QString::fromLatin1(qtscript_QSqlRelationalTableModel_function_names[i+1]),
            fun, QScriptValue::SkipInEnumeration);
    }

    engine->setDefaultPrototype(qMetaTypeId<QSqlRelationalTableModel*>(), proto);

    QScriptValue ctor = engine->newFunction(qtscript_QSqlRelationalTableModel_static_call, proto, function_lengths[0]);
