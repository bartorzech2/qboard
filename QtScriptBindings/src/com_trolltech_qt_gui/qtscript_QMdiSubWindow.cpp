...
QScriptValue qtscript_create_QMdiSubWindow_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 13; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QMdiSubWindow_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QMdiSubWindow_static_call, proto, function_lengths[0]);
