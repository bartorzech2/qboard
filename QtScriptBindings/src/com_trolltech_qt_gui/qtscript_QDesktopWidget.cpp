...
static QScriptValue qtscript_QDesktopWidget_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    case 1:
    if (context->argumentCount() == 0) {
        bool _q_result = _q_self->isVirtualDesktop();
    ...
    case 3:
    if (context->argumentCount() == 0) {
        int _q_result = _q_self->primaryScreen();
        return QScriptValue(context->engine(), _q_result);
    }
    break;
...
QScriptValue qtscript_create_QDesktopWidget_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 8; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QDesktopWidget_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QDesktopWidget_static_call, proto, function_lengths[0]);
