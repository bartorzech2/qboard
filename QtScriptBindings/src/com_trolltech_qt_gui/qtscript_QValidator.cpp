...
QScriptValue qtscript_create_QValidator_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 5; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QValidator_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QValidator_static_call, proto, function_lengths[0]);
