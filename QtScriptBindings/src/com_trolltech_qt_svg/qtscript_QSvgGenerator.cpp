...
static QScriptValue qtscript_QSvgGenerator_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (context->argumentCount() == 1) {
        ...
        _q_self->setOutputDevice(_q_arg0);
        return context->engine()->undefinedValue();
    }
    break;
...
QScriptValue qtscript_create_QSvgGenerator_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 9; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QSvgGenerator_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QSvgGenerator_static_call, proto, function_lengths[0]);
