...
QScriptValue qtscript_create_QSvgRenderer_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 10; ++i) {
        QScriptValue fun = engine->newFunction(qtscript_QSvgRenderer_prototype_call, function_lengths[i+1]);
    ...
    QScriptValue ctor = engine->newFunction(qtscript_QSvgRenderer_static_call, proto, function_lengths[0]);
